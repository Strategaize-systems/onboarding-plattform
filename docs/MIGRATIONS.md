...
### MIG-130 — V10.1 SLC-181 SOP-Brücke Provenance (LIVE-applied 2026-07-04 /deploy — postgres-User base64+ssh+psql; sop.source_modul_output_id + uq_sop_source_modul_output verifiziert)
- Date: 2026-07-04
...
### MIG-129 — V10.1 StB-Scoring-Flag-Seed (17 Fachmodule inkl. M-04) (LIVE-applied 2026-07-03 /backend SLC-177 — base64+ssh+psql-postgres; 17 Rows live-verifiziert, flagged-counts exakt)
- Date: 2026-07-03
- Scope: Reiner Daten-Seed (KEIN DDL): `UPDATE` der 17 `stb_modul_*`-`template`-Rows (v1.0) — setzt in `blocks[].questions[]` die 5 flachen Scoring-Flags (`owner_dependency`/`deal_blocker`/`sop_trigger`/`ko_hart`/`ko_soft`) auf die Founder-approvten Werte (bisher alle `false`). 233 geflaggte Fragen / 305 true-Flag-Felder über 337 Fragen. Content (Fragetext/Themen/KI-Hebel/Question-Counts/metadata) unverändert — Voll-Rebuild der `blocks` aus einem frozen Live-Snapshot (`docs/stb-vertikale/module-blocks-snapshot.json`) mit Flag-Overlay → beweisbar nur-Flags-Diff (kein `jsonb_set`-Pfad-Risiko, R-177-1). `NOTIFY pgrst`.
- Affected Areas: `public.template` (17 Rows, nur `blocks`-Flag-Felder + `updated_at`). Keine Schema-/Constraint-/Policy-Änderung. Kein RLS-Impact.
- Reason: Die bisher toten 5 Scoring-Flags (`TemplateQuestionSchema`, überall `false`) mit Leben füllen — Grundlage für die V10.1-Runtime-Delivery (Modul-Reife-Ampel SLC-178, Live-Haiku-Scoring SLC-179, SOP-Brücke SLC-181). Flags via Sonnet-4-Klassifikation (Bedrock Frankfurt eu-central-1, `/module-delivery`-Skill, DEC-253/E), Founder-Abnahme „alles ok" 2026-07-03. Deterministisch erzeugt: `docs/stb-vertikale/gen-mig129-flag-seed.py` (liest Snapshot + `module-delivery-flags.json`, uuid5-stabil, Re-Run byte-identisch).
- Risk: Sehr niedrig (Daten-Seed, kein DDL, idempotent — reines `UPDATE`, Feature-Flag NEXT_PUBLIC_ENABLE_STB_VERTIKALE OFF → 0 Prod-Impact). DB-Sidecar (node:20, Coolify-DB, withTestDb ROLLBACK) 4/4 PASS: Flags==approved (305 true-Felder), Content byte-identisch, idempotent. Live-verifiziert 2026-07-03: 17 Rows, flagged-counts 12/14/15/21/15/17/17/9/9/16/14/17/16/6/15/9/11, question-counts unverändert (337).
- Rollback Notes: Flags zurücksetzen = neuer Seed mit leerer Flag-Map (oder `UPDATE`, der alle Flag-Felder auf `false` setzt). Kein FK/Datenverlust (nur boolean-Overlay). Feature OFF → kein User-Impact bei Rollback.

### MIG-128 — V10 StB-Vertikale Fachmodul-Seed (16 Module Welle 3-5) (LIVE-applied 2026-07-02 /backend SLC-170b — base64+ssh+psql-postgres; 17 stb_modul_* Rows live-verifiziert)
- Date: 2026-07-02
- Scope: Idempotenter Seed von 16 `template`-Rows (`stb_modul_m01/02/03/06/07/08/15/16/26/27/28/35/36/38/39/42` v1.0) — der restliche StB-KERN-Cut (M-04=MIG-125, M-BP=MIG-126 separat). Je Row 2 Capture-Blocks (`stufe1_kern` required=true + `stufe2_vertiefung` required=false) + `metadata` (modul_key, modul_kategorie, output_contract Triple, themenmodell 6 Bereiche mit unterpunkte[], ki_hebel Reifegrad 1-4). Summe 311 Fragen + 146 KI-Hebel. Shape 1:1 zu MIG-125 (M-04): template-queries.ts (TemplateBlock/Question) + module-context.ts (ModuleMetadataSchema, Zod-Mirror gruen). Scoring-Flags owner_dependency/deal_blocker/sop_trigger/ko_hart/ko_soft = false (Delivery-Schicht /module-delivery). KEIN Schema-DDL. `INSERT ... ON CONFLICT (slug, version) DO UPDATE` (idempotent).
- Affected Areas: `public.template` (16 neue Rows, slugs `stb_modul_*`). Keine Schema-/Constraint-/Policy-Aenderung. Voraussetzungen live: template.metadata (MIG-093), UNIQUE(slug,version) (MIG-096), template-RLS/GRANTs (MIG-021/022).
- Reason: Founder-IP-Fachmodule (abgenommene Quellen `docs/stb-vertikale/M-<xx>-seed-source.md`, Modus A /module-author, DEC-234 neuer StB-Inhalt) seeden, damit Modul-Capture + module_output_synthesis (SLC-173/174) fuer den ganzen 18-Cut lauffaehig sind. Deterministisch erzeugt: `docs/stb-vertikale/gen-mig128-fachmodule-seed.py` (PARST die 16 seed-source.md §1/§3/§4/§5, committet, reproduzierbar, uuid5 slug-haltiger NS pro Modul, json.dumps ensure_ascii=False).
- Risk: Niedrig (Daten-Seed, kein DDL, idempotent, Feature-Flag NEXT_PUBLIC_ENABLE_STB_VERTIKALE OFF). Block-/Question-UUIDs deterministisch. Live-verifiziert 2026-07-02: 17 stb_modul_* Rows (16 neu + m04 aus MIG-125), je blocks=2, question/hebel-counts exakt (17 Standard / 16 M-42 / 24 M-06/26/27/28/35 / 22 M-07 / 26 M-04), modul_key gesetzt; Row-Count stabil (idempotent).
- Rollback Notes: `DELETE FROM public.template WHERE slug LIKE 'stb_modul_m%' AND slug <> 'stb_modul_m04' AND version='1.0';` (keine FK-Abhaengigen solange keine capture_session daran gebunden; Feature OFF).

### MIG-127 — V10 SLC-172 Blueprint-Diagnostik: atomarer KU-Seed-RPC (`rpc_seed_blueprint_diagnosis_input`) (LIVE-applied 2026-06-29 /deploy V10, RPT-540 — base64-frei via ssh-stdin + psql -U postgres; Function + `checkpoint_type`-CHECK + service_role-GRANT live-verifiziert. Geschrieben + DB-Sidecar-verifiziert 2026-06-24 /backend SLC-172 MT-2, RPT-533, DEC-249/250)
- Date: 2026-06-24
...
### MIG-132 — V10.4 strategaize_berater: Zuordnungs-Tabelle + Rolle + Sichtbarkeits-Helper (APPLIED LIVE 2026-07-08 /deploy REL-041)
- Date: 2026-07-08 (**APPLIED LIVE 2026-07-08 im /deploy REL-041**, Pre-Apply-Dump `/root/mig132_rollback/`; Verify: role-CHECK 5 Werte, Tabelle+2 Policies+2 Index, 3 Functions, idempotent, survived Compose-Recreate, PostgREST 401. Datei geschrieben /backend SLC-188 2026-07-07). **In-tx-ROLLBACK-Validierung 2026-07-08 gegen Live-DB PASS**: alle DDL sauber; handle_new_user legt strategaize_berater ohne tenant_id an; berater_assigned_tenant_ids Cardinality=2 (Kanzlei+Cascade-Mandant, Fremd nicht geleakt); can_see_tenant korrekt; role-CHECK 5 Werte. MT-1-Spike: partner_client_mapping(accepted) ≡ parent_partner_tenant_id (0 Divergenz) → Cascade nur via mapping; on_auth_user_created live vorhanden (R-ARCH-V-1/V-2 aufgelöst).
- Scope: (1) CREATE TABLE berater_tenant_assignments(berater_user_id uuid→profiles ON DELETE CASCADE, tenant_id uuid→tenants ON DELETE CASCADE, assigned_by uuid→profiles, assigned_at timestamptz DEFAULT now(), PK(berater_user_id,tenant_id)) + Index(tenant_id) + RLS enable + Policies (strategaize_admin ALL, strategaize_berater SELECT WHERE berater_user_id=auth.uid()) + REVOKE anon. (2) ALTER profiles role-CHECK 4→5 (add strategaize_berater). (3) CREATE OR REPLACE FUNCTION handle_new_user() — +strategaize_berater valid, berater im tenant_id-NICHT-Pflicht-Zweig (cross-tenant wie admin). (4) CREATE FUNCTION berater_assigned_tenant_ids(uuid) SECURITY DEFINER — zugewiesene ∪ Mandanten via partner_client_mapping(accepted). (5) CREATE FUNCTION can_see_tenant(uuid) SECURITY DEFINER — admin OR tenant ∈ berater_assigned_tenant_ids(auth.uid()).
- Reason: Rollenmodell V2 Paket P2 (DEC-263/267/268/269) — ausführende Berater-Ebene mit zuweisungs-gescopter Sichtbarkeit (BS-Port MIG-033/034/035 → OP tenant-scoped).
- Affected Areas: profiles (CHECK), auth-Trigger handle_new_user, neue Tabelle berater_tenant_assignments, 2 neue Sichtbarkeits-Functions. KEINE Änderung an den 40 tenant-scoped RLS-Policies in P2 (DEC-269, Query-Layer-Durchsetzung; via-assignment-RLS erst P4).
...
- Affected Areas: capture_session (Trigger + Column-DEFAULT + Tier-Gating), berater_assigned_tenant_ids (Body), profiles (neuer Guard-Trigger). Tier-Gating-Semantik: unspezifizierter tier = 'free' (least privilege) → App-Session-Creation muss entitled tier explizit via service_role setzen (BLOCKING MT-0, SC-V20-2).
...
- Affected Areas: ~31 DEFINER-Funktionen (RPC-Layer 032/047/054/062 u.a.). Reine Ausführungs-Kontext-Härtung, kein Verhaltens-Change.
- Risk: minimal (ALTER FUNCTION natürlich idempotent, kein DROP/CREATE). Umfang ist Heuristik → exakte Liste = Live-Sweep-Ergebnis (R-1). Done-Gate: Rest-Count 0 (SC-V20-3, Live-Verify).
- Rollback Notes: `ALTER FUNCTION … RESET search_path;` je betroffener Funktion (Pre-Apply-Liste dumpen). Kein Datenrisiko.
