...
### MIG-127 — V10 SLC-172 Blueprint-Diagnostik: atomarer KU-Seed-RPC (`rpc_seed_blueprint_diagnosis_input`) (RESERVIERT 2026-06-24 /architecture, DEC-249)
- Date: 2026-06-24
- Scope: Neue PL/pgSQL-Function `rpc_seed_blueprint_diagnosis_input(p_session_id uuid) → jsonb` (SECURITY DEFINER, GRANT service_role + ggf. authenticated/Owner). Liest `template.diagnosis_schema` + `capture_session.answers` der Blueprint-Session und seedet pro Diagnose-Block A–G in EINER Transaktion: je Frage in `diagnosis_schema.blocks[X].subtopics[].question_keys` eine `knowledge_unit` (`block_key=X`, `unit_type='observation'`, `source='questionnaire'`, `status='accepted'`, `confidence='medium'`, `title`=Fragelabel, `body`="Frage: …/Antwort: …") + die `block_checkpoint`-Referenz fuer den `diagnosis_generation`-FK (Reuse des bestehenden Capture-Checkpoints ODER thin per-A–G-Checkpoint je nach `block_diagnosis`/`rpc_create_diagnosis`-Constraint, im /backend zu fixieren). Return: `{ blocks: [{block_key, checkpoint_id, ku_count}], ... }`. Idempotent (Guard auf bestehende KUs/Checkpoints pro (session, block_key) — Re-Run completed nur Fehlendes). KEIN Schema-DDL (kein neues Table/Column), nur Function.
- Affected Areas: `public.knowledge_unit` (INSERT, A–G), `public.block_checkpoint` (INSERT/Reuse), neue Function `rpc_seed_blueprint_diagnosis_input`. Keine Constraint-/Policy-Aenderung. Voraussetzungen live: KU/Checkpoint-Schema (MIG-021), `template.diagnosis_schema` (MIG-051), `stb_blueprint_kanzlei`-Seed (MIG-126).
...
- Risk: Niedrig (additive Function, idempotent, kein DDL/Constraint-Touch). Live-Apply via `sql-migration-hetzner.md` (base64+ssh+psql-postgres) im /deploy; DB-Sidecar im /qa (node:22 + SAVEPOINT, coolify-test-setup.md): A–G-KU-Count + Idempotenz-Re-Run + Tenant-RLS.
