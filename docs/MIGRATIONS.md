...
### MIG-029 — V4.2 Self-Service Onboarding (`tenants.onboarding_wizard_*` + `reminder_log` + `user_settings`) (Migration 080, geplant)
- Date: geplant fuer SLC-046 + SLC-048 /backend (V4.2-Implementation)
...
- Live-Deploy: NICHT live. Geplant fuer SLC-046 (tenants ALTER) + SLC-048 (reminder_log + user_settings). Variante A: alle drei Bloecke in einem File `sql/migrations/080_v42_self_service.sql` deployt zu Beginn von SLC-046 (Empfehlung: einfacher Single-Run). Variante B: zwei Files (`080a_tenants_wizard.sql` fuer SLC-046, `080b_reminders_settings.sql` fuer SLC-048). Entscheidung in /slice-planning.
