...
- Reason: V10.1 FEAT-096 (Phase 1) — die 5 Scoring-Flags sind bis dato ueberall `false` + ungenutzt; die Delivery-Engine (Live-Bewertung P2 + Reife-Ampel P1-Runtime) braucht kuratierte Flag-Werte. Founder-Praemisse: Flags NIE handgesetzt → LLM (Sonnet) klassifiziert im `/module-delivery`-Skill, Founder nimmt ab, Ergebnis wird deterministisch geseedet (DEC-252/253).
- Affected Areas: `public.template` (17 Rows, nur `blocks`-JSONB-`flags`-Felder). Keine Schema-/Constraint-/RLS-/Policy-Aenderung. Modul-Content (Fragen/Themenbaeume/KI-Hebel) UNVERAENDERT (M-04-treu, DEC-251). Voraussetzung live: MIG-125 (M-04) + MIG-128 (16 Module) geseedet.
- Risk: Niedrig (Daten-Seed, kein DDL, idempotent, Feature-Flag `NEXT_PUBLIC_ENABLE_STB_VERTIKALE` OFF). Haupt-Risiko = Flag-Qualitaet (Sonnet zu aggressiv) → Founder-Abnahme-Gate + Prompt-Guardrails (R2/F-C). DB-Sidecar-Verify (App-Zod, Muster RPT-542): Flags gesetzt + Modul-Content unveraendert. Live-Apply via `sql-migration-hetzner.md`.
- Rollback Notes: Re-Seed mit allen Flags `false` (Pre-V10.1-Stand) via idempotentem Generator-Re-Run, ODER `template`-`blocks`-JSONB aus pg_dump-Backup zuruecksetzen. Reversibel (reiner Daten-Seed).
