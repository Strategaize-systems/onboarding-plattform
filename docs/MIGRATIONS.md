...
### MIG-044 — V7.1 SLC-136 text_override + text_override_history Tabellen (Migration 101, planned)
- Date: 2026-05-20
- Scope: Neue Tabellen `text_override` (UNIQUE auf scope+scope_id+text_key+locale, CHECK scope IN ('global','template','partner'), CHECK scope_id_matches_scope) + `text_override_history` (Audit-Log, INSERT-only) + RLS-Policies + GRANTs auf service_role + authenticated + Indizes auf (text_key, locale) und (scope, scope_id).
- Reason: V7.1 FEAT-055 Inline-Text-Override-Foundation (DEC-140). User-Direktive 2026-05-20 nach SLC-700-Live-Test: alle User-sichtbaren Texte editierbar ohne Code-Deploy.
- Affected Areas: Neue Tabellen, keine Aenderung an bestehenden Tabellen. RLS-Helper-Functions `is_strategaize_admin(uuid)` + `partner_admin_view` + `current_tenant_id()` werden referenziert (existieren seit V6 Migration 090).
- Risk: GRANTs muessen fuer service_role + authenticated korrekt gesetzt sein (siehe `feedback_migration_rls_needs_grants.md` IMP-Lehre aus OP V7 SLC-134). Sonst Production-Outage durch Permission-Denied beim ersten Save-Action-Call. CHECK-Constraint scope_id_matches_scope ist Pflicht (sonst inkonsistente Rows wie scope=global, scope_id=<UUID>).
- Rollback Notes: `DROP TABLE text_override_history; DROP TABLE text_override;` reversibel. Saves seit Live-Apply bleiben jedoch verloren (kein Backup-Restore noetig wenn vor V7.1-Deploy gerollback).

### MIG-045 — V7.1 SLC-138 template.questions[].helper_text + examples_md Schema-Erweiterung (Migration 099, planned)
- Date: 2026-05-20
- Scope: Schema-additive Erweiterung am `template.blocks[].questions[]` JSONB-Array. Neue optionale Felder pro question: `helper_text` (max 300 chars) + `examples_md` (max 800 chars, Markdown-Subset). Bestehende Templates unveraendert. Separate Migration 099a seeded Initial-Content fuer alle 24 Fragen des `partner_diagnostic v1`-Templates.
