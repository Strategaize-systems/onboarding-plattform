...
- Progress 2026-04-24 (SLC-033 code done): 8 Migrations-Files geschrieben und committet (`065_employee_role.sql`, `066_employee_invitation.sql`, `067_capture_mode_v4.sql`, `068_bridge_tables.sql`, `069_template_v4_fields.sql`, `070_handbook_snapshot.sql`, `071_handbook_storage_bucket.sql`, `075_rls_employee_perimeter.sql`). RPC-Migrationen `072_rpc_employee_invite.sql`, `073_rpc_bridge.sql`, `074_rpc_handbook.sql` folgen in SLC-034/035/039. Hetzner-Anwendung via sql-migration-hetzner.md-Pattern steht aus (base64 + `psql -U postgres`, kein `supabase_admin`). Zusaetzlich: Migration 065 hebt auch die Whitelist in `handle_new_user()` um `'employee'` (Deviation-Rule 2 — der Auth-Trigger wuerde sonst `rpc_accept_employee_invitation` in SLC-034 blockieren). `sql/schema.sql` und `sql/functions.sql` Init-Script-Parity gemacht (DEC-002).
