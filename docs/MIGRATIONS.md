...
### MIG-037 — V6.3 SLC-105 Diagnose-Werkzeug Schema-Erweiterung + Template-Seed (Migration 093, geplant)
- Date: 2026-05-16
- Scope:
...
- Live-Deploy: **GEPLANT in /backend SLC-105 MT-2.** Apply-Pattern per `sql-migration-hetzner.md`: lokal `base64 -w 0 sql/migrations/093_v63_partner_diagnostic_seed.sql`, ssh-payload `echo '<BASE64>' | base64 -d > /tmp/093_v63.sql && docker exec -i supabase-db-bwkg80w04wgccos48gcws8cs-<suffix> psql -U postgres -d postgres < /tmp/093_v63.sql`. Post-Apply-Verifikation: `SELECT column_name FROM information_schema.columns WHERE table_name='template' AND column_name='metadata'` = 1 Row, `SELECT slug, version, metadata->>'usage_kind', jsonb_array_length(blocks) FROM template WHERE slug='partner_diagnostic'` = `('partner_diagnostic', 'v1', 'self_service_partner_diagnostic', 6)`, `(SELECT COUNT(*) FROM jsonb_array_elements((SELECT blocks FROM template WHERE slug='partner_diagnostic')) b, jsonb_array_elements(b->'questions') q)` = 24.
