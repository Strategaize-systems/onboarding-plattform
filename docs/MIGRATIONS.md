...
### MIG-111 — V9.5 Cross-Thread-Synthese-Stage Schema (email_synthesized_unit + Status + Cost-Spalte)
- Date: 2026-06-12
- Scope: SKIZZE (LIVE-Apply ist /backend SLC-V9.5-B). Geplante `sql/migrations/119_v95_synthesis_stage.sql`. (1) `email_bulk_run.status` CHECK um `'synthesizing'`, `'synthesized'` erweitert (Drop+Add, zwischen `pattern_extracted` und `curating`). (2) `email_bulk_run.synthesis_cost_eur numeric(8,4) NOT NULL DEFAULT 0` neu + GENERATED-Spalte `total_cost_eur` DROP+RECREATE auf `(pre_filter_cost_eur + pattern_extraction_cost_eur + synthesis_cost_eur) STORED`. (3) NEUE Tabelle `email_synthesized_unit` (id, tenant_id FK tenants, bulk_run_id FK email_bulk_run ON DELETE CASCADE, title, description, evidence_snippets jsonb, themes text[], aggregated_confidence numeric(3,2), evidence_count int, suggested_section, curation_status CHECK(pending_curation|accepted|rejected|edited) DEFAULT 'pending_curation', curated_section, curator_user_id FK auth.users, curated_at, imported_to_handbook_at, imported_knowledge_unit_id FK knowledge_unit ON DELETE SET NULL, created_at). (4) NEUE Join-Tabelle `email_synthesized_unit_source` (id, synthesized_unit_id FK ON DELETE CASCADE, pattern_id FK email_pattern ON DELETE CASCADE, thread_id uuid, tenant_id) + UNIQUE(synthesized_unit_id, pattern_id). (5) RLS-Matrix analog MIG-106 (strategaize_admin SELECT cross-tenant; tenant_admin SELECT/INSERT/UPDATE own-tenant via auth.user_role()+auth.user_tenant_id()) + GRANTs authenticated/service_role + Indizes (bulk_run_id), (bulk_run_id, curation_status), (tenant_id).
...
- Affected Areas: `email_bulk_run` (Status-CHECK + 1 neue Spalte + GENERATED-Recreate), 2 neue Tabellen + RLS. Bestehende `email_pattern`/`email_message`/`email_thread` strukturell unveraendert. Curation-UI + `importAcceptedPatterns` lesen kuenftig `email_synthesized_unit` (Code, nicht Schema).
- Risk: GENERATED-Spalten-DROP+RECREATE auf `total_cost_eur` muss bestehende Werte korrekt re-berechnen (STORED → wird bei Recreate neu materialisiert; pre-existing Runs erhalten synthesis_cost_eur=0 → total bleibt korrekt). Status-CHECK-Erweiterung darf keine Run mit Bestands-Status brechen (alle Bestandswerte bleiben erlaubt). RLS-Pen-Test in /qa (SC-V9.5-8).
- Rollback Notes: `DROP TABLE email_synthesized_unit_source; DROP TABLE email_synthesized_unit;` + `email_bulk_run` Status-CHECK auf MIG-106-Stand zuruecksetzen + `total_cost_eur` GENERATED auf `(pre_filter + pattern_extraction)` zuruecksetzen + `DROP COLUMN synthesis_cost_eur`. Reversibel solange keine `synthesized`-Runs existieren (sonst Status-Backfill noetig).
