...
### MIG-132 — V10.4 strategaize_berater: Zuordnungs-Tabelle + Rolle + Sichtbarkeits-Helper (Datei geschrieben SLC-188 /backend 2026-07-08; Live-Apply = /deploy)
- Date: 2026-07-07 (Datei `sql/migrations/132_v104_berater_foundation.sql` geschrieben /backend SLC-188; **NOCH NICHT prod-applied** — Live-Apply im /deploy V10.4, Pre-Apply-Live-Audit Pflicht). **In-tx-ROLLBACK-Validierung 2026-07-08 gegen Live-DB PASS**: alle DDL sauber; handle_new_user legt strategaize_berater ohne tenant_id an; berater_assigned_tenant_ids Cardinality=2 (Kanzlei+Cascade-Mandant, Fremd nicht geleakt); can_see_tenant korrekt; role-CHECK 5 Werte. MT-1-Spike: partner_client_mapping(accepted) ≡ parent_partner_tenant_id (0 Divergenz) → Cascade nur via mapping; on_auth_user_created live vorhanden (R-ARCH-V-1/V-2 aufgelöst).
