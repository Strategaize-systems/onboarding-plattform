...
- Current Phase: V4.3 Released + Post-Launch-Window 24-48h laeuft. V4.4 /requirements + /architecture + /slice-planning done 2026-05-05 (RPT-151+152+153). **SLC-061 done** — /backend (RPT-154) + /qa PASS (RPT-155). **SLC-062 /backend code-complete + Live-Apply 2026-05-05** (RPT-156) — MIG-030 deployed auf Hetzner-Coolify, 25 Replace-Statements, ~41 Vorkommnisse korrigiert, Post-Audit 0 TRUE-POSITIVE, Idempotenz-Test PASS (2. Apply md5-stable). Pre-Apply-Backup als CSV in /opt/onboarding-plattform-backups/. V4.4 2/2 Slices /backend done — wartet auf /qa SLC-062. Branch direkt auf `main`, kein Worktree.

## Immediate Next Steps
1. **/qa SLC-062** — Code-side QA (Migration-File-Review, audit-Helper-Review) + Live-Smoke (Demo-Template-Capture-Session anlegen, Block-Titel-Render mit korrekten Umlauten). AC-1..AC-7 verifizieren.
2. **Gesamt-V4.4-/qa** nach SLC-062 — beide Slices SLC-061+062 zusammen verifizieren, SC-V4.4-1..6 abhaken inkl. V4.3-Regression-Smoke (Reader, Help-Sheet, Cross-Search, Bridge-Edit, Jitsi-Meeting, FileUpload).
