...
- Current Focus: **V4.3 Go-Live CONDITIONAL GO 2026-05-05 (RPT-149) als REL-011-Draft.** Deploy-Commit `0843362`. Identische Logik wie V4.2 (RPT-129) und V4.1 (RPT-111): Release freigegeben mit allen Pre-V4.3-Residual-Risks dokumentiert + 0 V4.3-eingefuehrte Risks. Self-hosted Stack-Readiness 7/7 PASS (Compose-ENV vollstaendig, internal kong-URL fuer Server-Side, Healthcheck-Endpoint wired, Auth-Flow End-to-End via RPT-146 verifiziert). Pre-Production-Compliance-Gate explizit aufgeschoben (feedback_compliance_gate_later: Anwaltspruefung + Privacy-Page + Azure-EU + ISSUE-042-Pattern erst vor echtem Cutover). V4.3 bleibt Internal-Test-Mode bis dahin. Rollback-Pfad: ec311a7 (V4.2 REL-010 Stand) → Coolify-Redeploy, **DB-Rollback NICHT erforderlich** (kein Schema-Touch). REL-011-Draft in RELEASES.md eingefuegt — finalisiert in /deploy nach User-Coolify-Action. **Vorgaenger-Schritte:** Final-Check CONDITIONALLY READY (RPT-148), Gesamt-/qa PASS (RPT-147), Bundle-Live-Smoke (RPT-146). Build PASS mit Dummy-ENVs (8.9s Compile, 11.5s TS), `next build` produziert 39 routes inkl. `/api/help/[pageKey]` als Static. Compose-ENV-Block voll fuer alle V4.3-relevanten Vars (V4.1 ISSUE-027 Resolution traegt). Secrets-Hygiene PASS (secrets-onboarding.txt korrekt gitignored, keine hardcoded Secrets im Code, nur 3 ENV-Files getrackt: `.env.deploy.example` + `.env.local.example` + `scripts/generate-secrets.mjs` ohne Werte). npm audit: 9 Moderate alle Pre-V4.3 (3 prod via Next-bundled postcss = ISSUE-026 wontfix, 6 devDep via vitest+vite-Chain). Keine V4.3-eingefuehrten Risks. 0 Blocker, 0 High, 1 Medium (ISSUE-034 wizard-test-mock-drift carry-over aus /qa), 7 Low residual (alle dokumentiert in KNOWN_ISSUES.md). Ready fuer **/go-live**. **Nachfolgend Gesamt-/qa PASS 2026-05-05 (RPT-147).** Alle 10 SC-V4.3 erfuellt (9 PASS + 1 DEFERRED nicht-blockierend SC-V4.3-10 Berater-Help-Review-Track parallel zum Code-Release). Vitest gegen Coolify-DB **526/535 PASS**, 9 FAIL ausschliesslich in `wizard-actions.test.ts` durch Pre-V4.2-Test-Mock-Drift (V4.2 ISSUE-031-Fix `d1978ca` schaltete `wizard-actions.ts` auf `createAdminClient`, Test mockt nur `createClient` → Mock greift nicht — NICHT V4.3-Regression, Live-Code via V4.2-Browser-Smoke 2026-04-30 user-bestaetigt). Stub-Scan 0 Findings in 31 V4.3-Production-Files. Wiring 4/4 Haupt-Ketten verifiziert: Worker→Reader Anchor-Konsistenz, Cross-Snapshot-Search Pipeline, Scroll-Spy+Permalink, Help-API+Learning-Center. V4.3-Constraints alle eingehalten (kein Schema-Touch, kein neuer Cron, kein neuer Container, Spike timeboxed 3:17min). 0 Blocker, 0 High, 1 Medium (M1 wizard-actions Mock-Drift, in V4.4 oder Hotfix), 2 Low (Pre-existing SchemaKonsolidierung+Cross-Search-Score, beide bewusst). Ready fuer **/final-check**.
- Current Phase: V4.3 Go-Live CONDITIONAL GO. Naechste Phase: /deploy als REL-011 (User-Coolify-Action) → /post-launch nach 24-48h Stable-Window.

## Immediate Next Steps
1. **/deploy V4.3 als REL-011** — User-manueller Coolify-Deploy auf Commit `0843362`. Pflicht-Smoke: Container-Health, HTTP 307 Login-Redirect, /api/health, Reader+Cross-Search+Help-Sheet, error_log 0 errors 5min nach Deploy.
