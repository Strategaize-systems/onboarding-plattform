...
- Current Focus: **V5 Architecture done 2026-05-05 mit User-Sign-Off zu Q-V5-A..E.** Alle 5 Open Questions geklaert via DEC-074..078: (A) eigene Tabelle `walkthrough_session` mit FK zu `capture_session` (Pattern wie dialogue_session V3 DEC-026), (B) WebM/VP9+Opus only kein Server-Transcoding, (C) 30min Hard-Cap zweistufig (Browser MediaRecorder Auto-Stopp + DB CHECK `duration_sec <= 1800`), (D) Direct-Upload via Supabase Signed URL 15min TTL + Pflicht-Privacy-Checkbox vor Approve, (E) Mic-only kein Screen-Audio. Architektur-Output: ARCHITECTURE.md V5-Sektion (~430 Zeilen) mit 4 Datenfluessen (Aufnahme, Whisper, Berater-Review, Cleanup), 4-Rollen-RLS-Matrix (16 Faelle Pflicht-Gate), Storage-Bucket `walkthroughs` Setup, 3 Server Actions (`requestWalkthroughUpload`/`confirmWalkthroughUploaded`/`approveOrRejectWalkthrough`), Worker-Job-Type `walkthrough_transcribe`. MIG-031 als geplante 3-File-Migration (082+083+084) dokumentiert (idempotent, additiv). Slice-Empfehlung an /slice-planning: 4 Slices SLC-071..074, ~20-25 MTs, ~2 Wochen. V5.1-Architecture (Q-V5.1-A..C zu Bedrock-Modell, PII-Pattern, Original-vs-Redacted-Storage) bleibt offen — kommt nach V5-Live-Feedback. V4.4 Re-Check faellig 2026-05-06 ~14:18 parallel.
- Current Phase: V5 Architecture (Walkthrough-Mode MVP). Naechste Phase: /slice-planning V5 fuer Slice-Files SLC-071..074 + Slice-INDEX-Update + BL-077..080 → in_progress.

## Immediate Next Steps
1. **/slice-planning V5** — die 4 Slice-Empfehlungen aus ARCHITECTURE.md V5-Sektion in finale `slices/SLC-071..074-*.md` zerlegen, MTs nummerieren, slices/INDEX.md updaten, planning/backlog.json BL-077..080 → `in_progress`.
2. **V4.4 Re-Check** 2026-05-06 ~14:18 (ad-hoc HTTP-Smoke + error_log-Query, kein eigenes /post-launch noetig). Parallel zu /slice-planning moeglich.
3. **BL-076 Investigation** capture-reminders Cron-Idempotenz-Logik pruefen, ggf. Hotfix vor /backend SLC-071 oder als V5-Mid-Stream-Fix (User-Entscheidung offen).
