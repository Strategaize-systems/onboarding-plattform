...
- Current Focus: **V5 Option 2 Architecture done 2026-05-06 (RPT-171)** nach Re-Plan auf DEC-079. Alle 9 Open Questions Q-V5-F..N entschieden in 9 neuen DECs (DEC-080..088), dazu 3 strukturelle DECs (DEC-089 Inherited-Anchor auf Strategaize-Dev-System DEC-079, DEC-090 walkthrough_step bleibt standalone bis V5.1 Handbuch-Integration, DEC-091 V5-Foundation-DECs DEC-074..078 bleiben accepted ohne supersede). V5-Foundation (walkthrough_session-Tabelle, WebM/VP9-Codec, 30min-Hard-Cap, Direct-Upload, Mic-only) bleibt unveraendert. Neue Methodik-Schicht: 3-stufige Bedrock-Sonnet-Pipeline (PII-Redaction → Schritt-Extraktion → Auto-Mapping mit Bridge-Engine-Pattern Reverse-Direction) erweitert das Worker-Container um drei Job-Handler. Datenmodell erweitert um zwei neue Tabellen `walkthrough_step` + `walkthrough_review_mapping` (geplant in MIG-032: Migrations 085+086+087). Q-V5-F kritisch geloest via DEC-080 Self-Spawn-Pattern (employee spawnt eigene capture_session) — unblock fuer SLC-071 Browser-Smoke. Methodik-Review-UI (FEAT-040) mit Subtopic-Tree + Unmapped-Bucket + Select-Move-Pattern (DEC-086) + Confidence-Ampel (DEC-087) + Pflicht-Checkbox-Gate ersetzt Roh-Video-Review. **Naechster Schritt: /slice-planning V5 Option 2** — 7 Slices SLC-071-Routing-Patch / SLC-072 / SLC-PII / SLC-EXT / SLC-MAP / SLC-REV / SLC-CLN finalisieren.
- Current Phase: V5 Option 2 Architecture done (2026-05-06), Slice-Planning-Phase startet.

## Immediate Next Steps
1. **`/slice-planning V5 Option 2`** — finale 7-Slice-Schnitt mit MTs (gemaess ARCHITECTURE.md V5 Option 2 Addendum Tabelle): SLC-071-Routing-Patch (Self-Spawn-Pattern + AC-10/11/12 Smoke, ~2-3 MTs), SLC-072 Whisper-Worker unveraendert (~5 MTs), SLC-PII Migration 087 + Worker + Pattern-Library (~4-5 MTs), SLC-EXT Migration 085 + Worker (~4-5 MTs), SLC-MAP Migration 086 + Worker mit Bridge-Engine-Reuse (~3-4 MTs), SLC-REV Methodik-Review-UI (~6-7 MTs), SLC-CLN Registry+RLS-Matrix-48-Faelle+Cleanup (~4-5 MTs). Gesamt ~28-34 MTs / ~5-6.5 Tage.
2. **`/backend SLC-071-Routing-Patch`** — ERSTE Slice nach Slice-Planning: Self-Spawn-Pattern via `startWalkthroughSession`-Action implementieren, Routing umstellen `/employee/walkthroughs` + `/employee/walkthroughs/[id]/record`, AC-10/11/12 Browser-Smoke nachholen (BL-086 Q-V5-F-Fix wird damit aufgeloest).
3. **MIG-032 Apply** in den jeweiligen Pipeline-Slices: Migration 087 (Status+Source-Erweiterung) → SLC-PII; Migration 085 (walkthrough_step) → SLC-EXT; Migration 086 (walkthrough_review_mapping) → SLC-MAP. Pre-Apply-Backup pro Migration via base64-Pattern.
4. **48-Faelle-RLS-Test-Matrix** als Pflicht-Gate fuer SLC-CLN: walkthrough_session 16 + walkthrough_step 16 + walkthrough_review_mapping 16, Vitest gegen Coolify-DB mit SAVEPOINT-Pattern.
5. **PII-Pattern-Library** in `src/lib/ai/pii-patterns/index.ts` mit synthetischer Test-Suite (≥90% Recall, SC-V5-6).
6. **BL-076 Cron-Idempotenz-Hotfix** weiterhin als Mid-Stream-Slot zwischen Pipeline-Slices und SLC-CLN einplanen (Pattern-Vorlage fuer Cleanup-Cron-Erweiterung).
7. **BL-067 Berater-Help-Review** parallel via direkten Editor-Workflow.
8. **V5.1 /architecture** offen, kommt nach V5-Option-2-Release (DEC-090: V5.1 nutzt approved walkthrough_step als Input).
