...
- Current Focus: **V5 Option 2 SLC-077 /backend code-side DONE 2026-05-07 — Stufe 2 Schritt-Extraktion Pipeline-Code live + Migration 085 live appliziert auf Hetzner.** Pflicht-Gates Code-side: 16/16 Worker-Mock-Vitest gruen (5 strukturierte Fixtures + N=0 Edge + Codeblock-Fence-Variante + 7 Failed-Pfade + Cost-Ledger Silent-Failure-Pattern), Worker-Bundle 220.6kb (vs SLC-076 205.9kb = +14.7kb). Migration 085 (`walkthrough_step` Tabelle: 17 Columns + UNIQUE walkthrough_session_id+step_number, 4 Indizes inkl. 2 partial WHERE deleted_at IS NULL, 2 RLS-Policies SELECT/UPDATE 4-Rollen-Matrix, BEFORE UPDATE Trigger) live appliziert via base64-Pipe + `psql -U postgres`. KEIN INSERT-Policy fuer authenticated (Worker schreibt via service_role/BYPASSRLS). Worker-Handler `walkthrough_extract_steps` live im Code (commit pending User-Coolify-Redeploy): handle-extract-steps-job.ts mit Idempotency (DELETE before INSERT), step_number 1..N erzwungen, transcript_offset_start/end via originalText.indexOf(snippet) deterministisch berechnet, Zod-Schema-Validation, Bedrock-Codeblock-Fence-Stripping, Pipeline-Trigger advance extracting → mapping. Bedrock-Live-Quality-Test (Variante B) gegated via `ENABLE_STEP_QUALITY_TEST=1` mit 5 deutschen Walkthrough-Fixtures (Auftragsannahme/Reklamation/Onboarding/Monatsabschluss/Inventur) — wird in /qa SLC-077 ausgefuehrt. claim-loop um JOB_TYPE walkthrough_extract_steps + 13. Handler-Argument erweitert. ai_cost_ledger.role='walkthrough_step_extractor' bereits in Migration 088 (SLC-076 /qa) erlaubt — kein zusaetzlicher DB-Schritt noetig. **Live-Smoke wartet auf User-Coolify-Worker-Redeploy:** 3 walkthrough_session in `extracting` + 3 ai_jobs `walkthrough_extract_steps` `pending` werden automatisch durchgepickt. **Naechster Schritt: User-Coolify-Worker-Redeploy + /qa SLC-077** mit Live-Smoke + ENABLE_STEP_QUALITY_TEST=1 Bedrock-Run.
- Current Phase: V5 Option 2 Implementation — SLC-077 code-side done (4/9 V5-Option-2-Slices done = SLC-071+075+072+076; SLC-077 jetzt 5/9 nach Live-Smoke), naechste Stufe SLC-078 Auto-Mapping.

## Immediate Next Steps
1. **User-Coolify-Worker-Redeploy** auf den SLC-077-/backend-Commit. Direkt danach werden die 3 pending walkthrough_extract_steps ai_jobs automatisch gepickt (Live-Smoke ohne weiteres Setup).
2. **`/qa SLC-077`** — Live-Smoke gegen 3 queueete Sessions + ENABLE_STEP_QUALITY_TEST=1 Bedrock-Run gegen 5 deutsche Fixtures + Stub-Scan + Wiring-Verifikation.
