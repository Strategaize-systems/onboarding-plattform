...
- Current Focus: **V5 Option 2 SLC-076 DONE 2026-05-07 — Stufe 1 PII-Redaction Pipeline live + Live-Smoke 3/3 PASS + Bedrock-Recall SC-V5-6 PASS (350/350 = 100%) + ai_cost_ledger Re-Smoke verifiziert ($0.00293/Job).** Migration 087 + Migration 088 live appliziert (status-Maschine 11 Werte + knowledge_unit.source 10 Werte + ai_cost_ledger.role 14 Werte). Worker-Handler `walkthrough_redact_pii` live (commit ac38fc2). 3 walkthrough_session durchgelaufen `redacting → extracting` (1× isoliert PII, 1× End-to-End ab Whisper, 1× isoliert PII): jeweils Original-KU `walkthrough_transcript` + Redacted-KU `walkthrough_transcript_redacted` mit evidence_refs={original_kuId, walkthrough_session_id, recorded_by_user_id} entstanden, Pipeline-Trigger feuert korrekt, Folge-Job `walkthrough_extract_steps` queued (wartet auf SLC-077-Handler). Pipeline-Latenz: Whisper 10.3s + PII 2-50s, weit unter Slice-Spec-Soll <2min. Bedrock-Live-Recall (350 deutsche PII-Items, 7 Kategorien) **100% Recall pro Kategorie + global** nach 1 Iteration. 3 In-Phase-Bugs gefixt: (1) User-Trailer-Pollution (commit f275da5); (2) PREIS_BETRAG-Recall 0% (commit c395c2c); (3) ai_cost_ledger Silent-Failure (commit a3f4aff + Migration 088). Tests 24/24 Vitest gruen. RPT-182 (/backend) + RPT-183 (/qa Mixed-PASS verifiziert). Commits 6e36fa4 + f275da5 + c395c2c + 2a30ec5 + a3f4aff + ac38fc2 auf main; 3 IMPs (369/370/371) ins Dev-System. **Naechster Schritt: `/backend SLC-077`** Stufe 2 Schritt-Extraktion (MIG-085, 5 MTs, ~5h).
