...
- Current Focus: **V5 Option 2 SLC-078 /backend code-side DONE 2026-05-07 — Stufe 3 Auto-Mapping implementiert.** MT-1 Migration 086 LIVE auf Hetzner (DB-Container `supabase-db-...-111957618447`, base64-Pipe Apply, Pre-Apply-Greenfield-Check). 15 Columns inkl. **GENERATED `confidence_band`-Column** (NULL→red, ≥0.85→green, ≥0.70→yellow, ELSE red — DEC-087) + 4 Indizes + 4 FKs (tenants/walkthrough_step CASCADE, template, auth.users) + 2 RLS-Policies (4-Rollen-Matrix via JOIN auf walkthrough_step→walkthrough_session, KEIN INSERT-Policy fuer authenticated) + Trigger _set_updated_at. GENERATED-Logik live verifiziert via Test-INSERT (0.85 + valider unterbereich → 'green', Cleanup direkt). MT-2 Bedrock-Prompt `subtopic_map.ts` + Zod-Schema (IMP-369-disziplinkonform: Anweisungen System-Prompt-only). MT-3 Worker `handle-map-subtopics-job.ts` mit `buildSubtopicTree`-Helper (sop_trigger-Filter mit Fallback) + Bridge-Engine-Pattern-Konsistenz (gleicher chatWithLLM, gleiche Cost-Heuristik, role='walkthrough_subtopic_mapper' per MIG-088). Confidence-Threshold ENV `WALKTHROUGH_MAPPING_CONFIDENCE_THRESHOLD` Default 0.7 mit kaputt-ENV-Fallback. LLM-Halluzinations-Validation: erfundene subtopic_id-Strings → NULL gekippt. Idempotency via DELETE-before-INSERT. Wiring claim-loop.ts (12→13 JOB_TYPES) + run.ts (13. Argument). MT-4 Vitest 26/26 PASS in 26ms — 3 Fixtures Pflicht-Gate ≥70% pro Fixture (Coverage 80%/75%/83%), globale Coverage 80% (12/15) als Diagnose, 4 Confidence-Threshold-Tests, N=0, Skip, 9 Failed-Pfade, Cost-Ledger Silent-Failure (IMP-371), 5 buildSubtopicTree-Tests, LLM-laesst-Step-weg-Default. Worker-Bundle 237.9 KB (vs SLC-077 220.6 KB, +17.3 KB). TypeScript clean. ESLint 0/0 auf 7 neue+geaenderte Files. Walkthrough-Pipeline 65/65 Vitest gruen. **Architecture-Drift entdeckt + dokumentiert**: Architecture-Doc `template.blocks[].subtopics[]`-Annahme existiert nicht im Live-Schema; effektive Subtopic-Schicht = `blocks[].questions[].unterbereich`-String. DEC-085 Consequence ergaenzt + DEC-092 (Drift-Korrektur) angelegt + ARCHITECTURE.md V5 Option 2 an 2 Stellen korrigiert. **Naechster Schritt: User-Coolify-Worker-Redeploy auf SLC-078-Commit** → 3 pending `walkthrough_map_subtopics`-Jobs aus SLC-077-Smoke werden automatisch durchgepickt → /qa SLC-078.
- Current Phase: V5 Option 2 Implementation — SLC-078 /backend done (6/9 V5-Option-2-Slices code-done = SLC-071+075+072+076+077+078), naechste Live-Smoke + /qa SLC-078, dann SLC-079 Methodik-Review-UI.

## Immediate Next Steps
1. **User-Coolify-Worker-Redeploy auf SLC-078-Commit** → Boot-Log soll `walkthrough_map_subtopics handler registered` enthalten. 3 queueten ai_jobs `walkthrough_map_subtopics` werden direkt nach Handler-Start gepickt (alle 3 sind N=0 aus SLC-077-Smoke → Pipeline mapping → pending_review ohne Bedrock-Call). Plus 1 Happy-Path-Session (f51951ca, 4 walkthrough_step-Rows aus SLC-077-Smoke) — wenn die noch in mapping ist, wird sie ebenfalls gepickt und ergibt einen echten Bedrock-Mapping-Smoke.
2. **`/qa SLC-078`** mit: Live-Smoke-Verifikation (alle queuete + injizierte Sessions wandern mapping → pending_review, walkthrough_review_mapping-Rows persistiert mit confidence_band-Verteilung, 0 Cross-Tenant-Mappings), Stub-Scan, ai_cost_ledger fuer role='walkthrough_subtopic_mapper', Wiring-Chain. Optional Live-Bedrock-Quality-Test gegated falls Confidence-Verteilung suspect.
3. **`/frontend SLC-079`** — Methodik-Review-UI (3 Admin-Routen + SubtopicTreeReview + UnmappedBucket + Move-Pattern + Pflicht-Checkbox + Cockpit-Card + RawTranscriptToggle + SC-V5-4 Berater-Smoke, ~7 MTs). Kann nach SLC-078-Live-Smoke starten — Methodik-Review nutzt walkthrough_step + walkthrough_review_mapping als Daten-Backend.
