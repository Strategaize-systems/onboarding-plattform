...
- Current Focus: **V5 Option 2 SLC-077 DONE 2026-05-07 — Stufe 2 Schritt-Extraktion live deployed + Live-Smoke 4/4 PASS + Bedrock-Quality-Test Variante B 6/6 PASS.** User-Coolify-Worker-Redeploy auf Commit 98d0625 durch (Container `worker-bwkg80w04wgccos48gcws8cs-111957607742`, DB `supabase-db-...-111957618447`); Boot-Log enthielt `walkthrough_extract_steps handler registered`. Live-Smoke: 3 SLC-076-Smoke-Sessions → N=0 Edge-Case live verifiziert (Bedrock korrekt `[]` bei Voice-Setup-Smalltalk-Aufnahmen, je $0.0032/1.6-2.1s) + 1 Happy-Path mit injiziertem Auftragsannahme-Body → 4 walkthrough_step methodisch korrekt extrahiert (Buchhaltung-Auftrag-anlegen / Vertriebsleiter-Bestaetigungs-E-Mail / Produktion-einplanen / Versand-dokumentieren), 0 PII-Halluzinationen, transcript_offset_start/end deterministisch via indexOf berechnet ($0.0078/4.7s). Bedrock-Quality-Test SC-V5-7-Anchor (Variante B): ENABLE_STEP_QUALITY_TEST=1 vs 5 deutsche Fixtures = 6/6 PASS in 32.8s ~$0.04 — alle Fixtures liefern ≥expectedMinSteps (23/17 Schritte total), 0 PII-Halluzinationen (0/23), 100% Action-Pattern-Coverage (17/17). 1 Test-Spec-Schwaeche entdeckt + gefixt (Regex umlautlos `pruef` → `pr[uü]f` Umlaut-Variante akzeptieren). Stub-Scan clean, Wiring-Chain 13/13 PASS. Tests 16/16 Vitest gruen lokal. RPT-184 (/backend) + RPT-185 (/qa). 3 walkthrough_session jetzt in `mapping` + 3 pending `walkthrough_map_subtopics`-Jobs warten auf SLC-078-Handler. **Naechster Schritt: `/backend SLC-078`** Stufe 3 Auto-Mapping (Migration 086 + Bridge-Engine-Pattern Reverse-Direction + Coverage-Test SC-V5-7 ≥70%, ~4 MTs).
- Current Phase: V5 Option 2 Implementation — SLC-077 done (5/9 V5-Option-2-Slices done = SLC-071+075+072+076+077), naechste Stufe SLC-078 Auto-Mapping.

## Immediate Next Steps
1. **`/backend SLC-078`** — Stufe 3 Auto-Mapping (Migration 086 + walkthrough_map_subtopics Worker mit Bridge-Engine-Pattern Reverse-Direction + Coverage-Test SC-V5-7 ≥70%, ~4 MTs). Picked vom bereits queueten ai_jobs walkthrough_map_subtopics (3 pending Jobs aus SLC-077-Smoke werden direkt durchlaufen sobald Handler live).
