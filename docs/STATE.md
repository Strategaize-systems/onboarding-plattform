...
- Current Focus: **/backend SLC-071 in progress (3/9 MTs done) — V5 Foundation.** MIG-031 voll live auf Hetzner Onboarding (159.69.207.29) am 2026-05-06: Migrations 082+083+084 strikt sequentiell deployed via base64-Pipe + `psql -U postgres`, jede einzeln verifiziert + idempotenz-getestet (Re-Apply 0 Drift). Pre-Apply-Backup `/opt/onboarding-plattform-backups/pre-mig-031_20260506_064103.sql` (schema-only, 7524 Zeilen, 236.593 bytes). Worktree `slc-071/v5-walkthrough-foundation` (Commit f7374b0). 082: capture_mode + knowledge_unit.source CHECK additiv erweitert. 083: walkthrough_session Tabelle (22 Cols) + 4 Indizes + RLS aktiv + 3 Policies + GRANTs + updated_at-Trigger. 084: storage.buckets `walkthroughs` privat + 500MB Limit + video/webm-only + 3 Storage-RLS-Policies. RLS-Translation-Note: ARCHITECTURE.md V5-Sketches verwendeten generisches JWT-Claims + tenant_user-Pattern, Migrations uebersetzten sauber auf produktiven `auth.user_role()` + `auth.user_tenant_id()` Helper aus sql/functions.sql (Architektur-Intent identisch — 4-Rollen-Matrix, Tenant-Isolation, Self-only fuer Mitarbeiter). Naechste Schritte: MT-4 (Server Action `requestWalkthroughUpload`) + MT-5 (`confirmWalkthroughUploaded` + ai_jobs-Queueing) als naechste Session. UI-MTs (MT-6+7+8) eigene Session danach. V4.4 Re-Check 2026-05-06 ~14:18 laeuft parallel ohne Konflikt.
- Current Phase: V5 Implementation (Walkthrough-Mode MVP, 1/4 Slices angefangen, Foundation-Migration live). Naechste Phase: /backend SLC-071 MT-4..MT-9 fortsetzen.

## Immediate Next Steps
1. **/backend SLC-071 MT-4 + MT-5** — Server Actions `requestWalkthroughUpload` + `confirmWalkthroughUploaded` in `src/app/actions/walkthrough.ts`, plus Vitest-Tests (6 Cases). Code-Pattern aus dialogue-session-actions.ts uebernehmen (auth.getUser + profiles-Query fuer tenant_id/role). Live-Smoke: Server-Action via Test-Page rufen, walkthrough_session-Eintrag entsteht; nach manuellem Browser-Upload + confirm → ai_jobs-Eintrag pending fuer SLC-072.
2. **V4.4 Re-Check** 2026-05-06 ~14:18 (ad-hoc HTTP-Smoke + error_log-Query, kein eigenes /post-launch noetig). Parallel zu /backend moeglich (verschiedene Live-Touchpoints).
3. **BL-076 Cron-Idempotenz-Hotfix** zwischen SLC-073 und SLC-074. **Nicht jetzt vorziehen** — User-Entscheidung 2026-05-05: Mid-Stream-Slot, weil thematische Naehe zum Cleanup-Cron-Setup in SLC-074. BL-076 bleibt `open` bis dann.
