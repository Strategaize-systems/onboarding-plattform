...
- Current Focus: **V4.3 SLC-054 /frontend Code-side done 2026-05-04 (RPT-144).** Letzter V4.3-Code-Slice durchgezogen: (a) `useSearchHistory()`-Hook mit pure helpers `addQueryToHistory` + `parseHistory` (10 Vitest-Cases PASS, SSR-safe via Mount-Effect, DEC-063 localStorage-only); (b) `searchAcrossSnapshots()` Cross-Snapshot-Engine mit Title-Hit-Boost (5x) + Recency-Weight (linear 7d→365d von 1.0→0.5) + maxResults-Cap 50 (14 Vitest-Cases PASS); (c) `ReaderSearchBox`-Komponente mit Sidebar-Integration (oben, Q-V4.3-H), Focus-zeigt-History, debounced 300ms Live-Search, ESC-Close, Click-Outside-Close, Click-Navigates + addQuery, Performance-Warning bei >20 Snapshots; (d) `loadCrossSearchSnapshots()`-Server-Helper mit Promise.allSettled + HARD_CAP=50 + ZIP-Reuse, fehlerhafte Snapshots werden uebersprungen; (e) page.tsx-Integration reicht crossSearchSnapshots an ReaderShell→ReaderSidebar durch. ESLint Errors react-hooks/set-state-in-effect: ReaderSearchBox-Debounce reworked (setState immer im Timeout-Callback), useSearchHistory-Hydration mit dokumentiertem disable. 24/24 neue Tests PASS, 386/386 ENV-unabh. Tests PASS (= Baseline 362 + 24 neue), TS+ESLint+Build clean. Browser-Smoke + LocalStorage-Persistenz-Smoke + Mobile 375x667 stehen aus → /qa SLC-054.
- Current Phase: V4.3 SLC-054 /frontend Code-side done. Naechste Phase: /qa SLC-054 (Browser-Smoke 1280×800 + 375×667 + LocalStorage-Persistenz + 4-Rollen-RLS-Matrix-Regression-Smoke) → Bundle-Live-Smoke V4.3 (alle 6 Slices) → Gesamt-V4.3-/qa → /final-check → /go-live → /deploy als REL-011.
