...
- Current Phase: V4.3 Released + Post-Launch-Window 24-48h laeuft. V4.4 /requirements done 2026-05-05 (RPT-151) — Maintenance-Sammelrelease als Pre-V5-Hygiene. Naechste Phase: /architecture V4.4 (4 Open Questions Q-V4.4-A..D).

## Immediate Next Steps
1. **/architecture V4.4** — 4 Open Questions klaeren: BL-068 FALSE-POSITIVE-Policy (Per-Item True/False), BL-069 Migration-Format (SQL `jsonb_set` empfohlen), BL-067 Review-Form (User editiert direkt empfohlen), Slice-Bundling (2 Slices empfohlen: SLC-061 Lint-Sweep + SLC-062 SQL-Backfill).
2. **User-Browser-Smoke V4.3** — Login + `/dashboard/handbook/<id>` Reader laden, Cross-Search-Box reachable, Help-Sheet-Tabs wechseln. Kann parallel zu /architecture V4.4 laufen.
