...
- Current Focus: **V5 Requirements done 2026-05-05 mit User-Sign-Off.** Walkthrough-Mode ist als 5. Capture-Modus geplant (parallel zu Questionnaire/Evidence/Voice/Dialogue). Tech-Stack-Decision: **Web-only via getDisplayMedia + getUserMedia** — kein Browser-Extension/Electron/Native (User 2026-05-05). Scope-Aufteilung: V5 = MVP-Capture + Berater-Review (manueller Approve, KEINE KI-Pipeline), V5.1 = AI-Pipeline (PII-Redaction + Schritt-Extraktion + Handbuch-Integration). Privacy-Verschaerfung: Roh-Aufnahmen sind nur fuer Berater + Aufnehmer sichtbar bis Approve, Pflicht-Checkbox vor Approve "keine kundenspezifischen oder sensitiven Inhalte sichtbar". 5 Features (FEAT-034..038), 5 BL-Items (BL-077..082), 7 Slices skizziert (SLC-071..077). Naechste Phase: /architecture V5 — klaert Q-V5-A..E (Datenmodell, Codec, Max-Dauer, Upload-Strategie, Audio-Mix). V4.3 Post-Launch STABLE + V4.4 Post-Launch INITIAL PASS (RPT-162) als Stabilitaets-Anker hinter uns. V4.4 Re-Check faellig 2026-05-06 ~14:18 (ad-hoc, kein eigenes /post-launch).
- Current Phase: V5 Requirements (Walkthrough-Mode MVP). Naechste Phase: /architecture V5 fuer 5 Open Questions Q-V5-A..E + Q-V5.1-A..C.

## Immediate Next Steps
1. **/architecture V5** — klaert: Datenmodell (eigene Tabelle vs. capture_session-Erweiterung), Storage-Codec (WebM/VP9 only oder MP4-Transcoding), Max-Dauer-Limit (Vorschlag 30min), Upload-Strategie (Direct vs. Server-Proxy), Audio-Mix (Mic-only empfohlen).
