...
- Current Focus: **V5 Option 2 SLC-075 /qa code-side + Live-Wiring PASS 2026-05-06 (RPT-174)**. Deploy-Commit `20eef2d` live healthy, Live-Container Image-Tag verifiziert, neue Routen im deployed Build registriert (`.next/server/app/employee/walkthroughs/{page,[id]/{page,record}}`), alte Route weg (`/employee/capture/walkthrough` not found im Image). Server-side AC-10 PASS (HTTP 307→/login). Wiring-Chain-Test gegen Coolify-DB PASS: atomare capture_session+walkthrough_session-Erzeugung via service_role + RLS Self-Only verifiziert (eigener employee sieht beide Rows, foreign user sieht 0 Rows). Stub-Scan 0 Treffer, Vitest 13/13 GREEN, lint 0/0. Pending: AC-11+AC-12 = User-Browser-Smoke (Permission-Dialog + Recording + Upload, nur per echter Browser-Gesture verifizierbar). Nach User-Smoke-PASS: SLC-071 + SLC-075 → done, BL-086 → done. **Naechster Schritt: User-Browser-Smoke `richard@bellaerts.de` auf https://onboarding.strategaizetransition.com/employee/walkthroughs.**
- Current Phase: V5 Option 2 Implementation (SLC-075 code-side + Live-Wiring done, awaiting User-Browser-Smoke).

## Immediate Next Steps
1. **User-Browser-Smoke AC-11+AC-12** auf https://onboarding.strategaizetransition.com/employee/walkthroughs (3 Schritte): (a) Liste + Button sichtbar [AC-10 visuell], (b) Klick "Neuen Walkthrough starten" → Permission-Dialoge (Bildschirm + Mikrofon) → Aufnahme startet [AC-11], (c) 5-10s aufnehmen → Stoppen → Upload-Progress → Status-Page mit `uploaded` [AC-12]. Bei PASS: Records-Final-Sync (SLC-071+SLC-075 → done, BL-086 → done). Bei FAIL: /doctor.
