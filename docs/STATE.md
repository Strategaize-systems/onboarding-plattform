...
- Current Phase: V4.3 Released + Post-Launch-Window 24-48h laeuft. V4.4 /requirements + /architecture done 2026-05-05 (RPT-151+152). 4 Decisions zementiert (DEC-070 Lint-Klassifikation, DEC-071 MIG-030-Format, DEC-072 BL-067-Review-Form, DEC-073 Slice-Bundling). MIG-030 skizziert. Naechste Phase: /slice-planning V4.4.

## Immediate Next Steps
1. **/slice-planning V4.4** — 2 Slice-Specs schreiben: SLC-061 Lint-Sweep (~6 MTs gegen die per DEC-070 klassifizierten 7 Errors + 6 Warnings) + SLC-062 SQL-Backfill (~3 MTs fuer audit-extract Wortliste + MIG-030 schreiben + Hetzner-Apply via base64). Geschaetzt 30 min.
2. **User-Browser-Smoke V4.3** — Login + `/dashboard/handbook/<id>` Reader laden, Cross-Search-Box reachable, Help-Sheet-Tabs wechseln. Kann parallel laufen.
3. **/post-launch V4.3** nach 24-48h Stable-Window — Container-Logs, error_log, 06.05. 09:00 Cron-Run `capture-reminders-daily` pruefen.
4. **/requirements V5** (Walkthrough-Mode) nach V4.4-Release. Capture-Mode-Hooks aus V4 sind ready. Vorher Technologie-Spike (Browser-Extension vs. Electron vs. Native).
