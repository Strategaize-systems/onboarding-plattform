...
- Next Action: Kleiner Backend-Slice — `listTemplates` defensiv machen (per-Row `safeParse`, ungueltige Rows skippen + `error_log`/captureInfo) UND/ODER Picker auf echte Capture-Templates filtern (Diagnose/Teaser via template-Purpose/-Slug-Whitelist ausschliessen). „Welche Templates duerfen im Picker erscheinen" = kleine Produktentscheidung (Founder). Optional zusaetzlich: 2 Legacy-Rows normalisieren oder aus `template` entfernen, falls sie nirgends sonst konsumiert werden (partner_diagnostic-Konsum pruefen).
