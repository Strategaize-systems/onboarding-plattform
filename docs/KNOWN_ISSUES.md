...
- Summary: `src/lib/handbook/get-review-summary.ts` filtert `block_review` und `knowledge_unit` zusaetzlich auf `capture_session_id`. Aufrufer (`/dashboard/page.tsx` Cockpit-Card und `/admin/handbook/page.tsx` TriggerHandbookButton) uebergeben die GF-Session des tenant_admin (z.B. `adbb827d-...`), aber `block_review.capture_session_id` und employee_questionnaire-KUs haben die Mitarbeiter-Session-ID (z.B. `22234f9e-...`). Resultat: getReviewSummary liefert immer `{approved:0, pending:0, rejected:0, totalEmployeeBlocks:0}` und damit zeigt die Cockpit-Card "—" / "Noch keine Mitarbeiter-Beitraege" statt korrektem X/Y, und der Quality-Gate-Confirm-Dialog erscheint nie weil `pending=0`.
- Impact: AC-7, AC-8, AC-10 von SLC-042 funktional gebrochen in Production. Quality-Gate-Workflow (DEC-045 weiches Gate) ist umgehbar — Berater triggert Snapshot ohne den Pending-Hinweis zu sehen, Audit-Field `pending_at_trigger` zeigt `0` statt der echten offenen Anzahl. /dashboard/reviews ist NICHT betroffen (filtert nur auf tenant_id). RPT-101 Problem #4 hatte das Verhalten als "intentional Low" markiert — Live-Smoke 2026-04-28 hat aber gezeigt, dass es ein echter Funktionsfehler ist.
- Workaround: Keiner. /dashboard/reviews zeigt korrekt; Cockpit-Card + Quality-Gate sind broken.
- Next Action: Fix in `getReviewSummary` — `captureSessionId` optional machen, Filter weglassen wenn nicht gesetzt. Aufrufer (`/dashboard/page.tsx`, `/admin/handbook/page.tsx`) ohne Session-ID aufrufen. Tests in `get-review-summary.test.ts` um 2 Faelle erweitern (mit/ohne Session). Build + 358 Tests gruen, User-Redeploy, Smoke-Punkt 4 + 5 + 6 wiederholen. Test-Daten in Live-DB sind angelegt (3 employee_questionnaire-KUs in Bloecken A/B/C in Session 22234f9e-..., Marker `QA-SMOKE-RPT-102`). Block A approved, Block B rejected, Block C pending — fuer Re-Smoke wiederverwendbar.
