...
- Next Action: **Code-Fix DONE 2026-07-09 (RPT-618)** — `listTemplates` (`src/lib/db/template-queries.ts`) macht jetzt per-Row `safeParse`: ungueltige Rows werden uebersprungen + via `captureWarning` (slug + zod-issues) geloggt, statt die ganze Seite zu werfen. Damit fallen die 2 Legacy-Rows automatisch aus dem Picker und die Loesung ist zukunftssicher gegen weiteren Legacy-Drift. 4 hermetische Vitest-Tests (`__tests__/template-queries-resilience.test.ts`), tsc 0 / eslint 0 / next build PASS / Full-Suite +4 pass, 0 Regression. **Wirksam erst ab naechstem /deploy** (OP hat keinen Auto-Deploy-Webhook) — bis dahin bleibt der Prod-Crash bestehen, daher Status noch `open`. Offene, bewusst NICHT mitentschiedene Produktfrage (separat, Founder): Sollen die `stb_modul_*`-Templates im Direkt-Client-Erhebungs-Picker ueberhaupt erscheinen? (Whitelist-/Purpose-Filter waere ein Folge-Schritt.) Optional: die 2 Legacy-Rows normalisieren/entfernen, falls nirgends sonst konsumiert.
