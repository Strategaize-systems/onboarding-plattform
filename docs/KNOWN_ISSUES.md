...
- Impact: Self-Upgrade zu Voll-Kunde (ISSUE-097-Klasse auf dem INSERT-Pfad offen) + unbounded Bedrock-Kosten auf Plattform-Rechnung. Kein Tenant-Isolation-/Auth-Bruch (bleibt im eigenen Tenant) → Severity Medium; ABER bypasses den purpose-built DEC-219-Fix → Prioritaet HIGH (zuerst im Hardening-Slice). 0 Impact heute (alle Sessions default handbook, Tiers unverkauft).
...
### ISSUE-127 — Keine globale CSP/COOP/COEP (Defense-in-Depth-Luecke, re-confirm V22/BL-138)
- Status: open
- Severity: Medium
...
- Impact: Info-Disclosure von Partner→Mandant-Zuordnung (nur UUIDs, keine Inhalte); braucht Kenntnis einer berater-uid; leer fuer non-berater. Low. NEU (MIG-132/V10.4).
...
- Status: resolved
- Severity: Blocker
- Area: V9.75 Tier-Gating Worker-Defense (`src/workers/condensation/claim-loop.ts` `evaluateWorkerTierGate`/`resolveSessionForGate`) + ungestempelte Dispatch-Sites (Migration 073 `rpc_trigger_bridge_run`, `src/app/api/capture/[sessionId]/evidence/upload/route.ts`, `src/workers/dialogue/handle-transcription-job.ts`)
- Summary: /qa SLC-V9.75-A entdeckt 2026-06-17 (RPT-484, Worktree `v9-75-exit-readiness` `c7fad17`). Drei gated job_types werden von Dispatch-Sites enqueued, die V9.75 NICHT angefasst hat → `ai_jobs.session_tier = NULL`. Die Worker-Defense faellt fuer NULL-Stempel auf `resolveSessionForGate(payload)` zurueck, kann aber keinen der drei Payloads aufloesen (kannte nur `email_bulk_*`/`bulk_run_id`, `payload.capture_session_id`, `payload.block_checkpoint_id`): `bridge_generation` (Payload `{bridge_run_id}`, via `rpc_trigger_bridge_run` 073), `evidence_extraction` (Payload `{evidence_file_id, session_id}` — Key `session_id` ≠ `capture_session_id`, via evidence-upload-route), `dialogue_extraction` (Payload `{dialogue_session_id}`, via `handle-transcription-job` Chain). Alle drei → `tier_gate_denied_worker`. Empirisch reproduziert (Probe `evaluateWorkerTierGate` → alle 3 false) + live bestaetigt (`fn_tier_allows(NULL, <job>) = false`).
- Impact: Ohne Fix waeren Bruecke-, Chef-Evidence- und Dialog-Extraktion nach Deploy fuer JEDE Session (auch entitelte `handbook`) im Worker gestorben → AC-A-8 „0 Regression" verletzt. Im Internal-Test-Mode (Founder-only) ohne Kunden-Impact, aber Merge/Deploy-Blocker.
- Resolution: 2026-06-17 (RPT-485, Worktree-Commit `892a7ee`). Schicht 1 — alle 3 Sites stempeln+gaten jetzt: `rpc_trigger_bridge_run` (073) als CREATE-OR-REPLACE in Mig 121 §4 (Gate blueprint + `session_tier`-Stempel); `evidence/upload/route.ts` (`assertSessionTierAllows` + Stempel, Enqueue uebersprungen statt 403); `handle-transcription-job.ts` (dialogue_extraction erbt `job.session_tier` des Eltern-Jobs). Zusatz: Schicht-1-Gate fuer `submitDiagnoseRun` (`knowledge_unit_condensation`). Schicht 2 — `resolveSessionForGate` loest jetzt `bridge_run_id` + `dialogue_session_id` auf (Backstop gegen kuenftig vergessenen Stempel). Verifiziert: tsc0/eslint0, 34 DB-Sidecar + 19 hermetisch GREEN (inkl. bridge-RPC-Gate+Stempel + 2 Backstop-Resolve-Tests), `next build` PASS.
- Next Action: Mit SLC-V9.75-A in `main` mergen (Pre-Merge-Re-Check) + Live-Apply Mig 121 im /deploy V9.75.
...
- Status: resolved
- Resolution: 2026-06-29 (V10 /deploy, RPT-540). Coolify-ENV korrigiert: `LLM_MODEL` + `BEDROCK_V9_SONNET_MODEL_ID` + `BEDROCK_V9_HAIKU_MODEL_ID` von roh (`claude-sonnet-4-20250514-v1:0`) auf das eu-Inference-Profile `eu.anthropic.claude-sonnet-4-20250514-v1:0` gesetzt + Redeploy. Post-Redeploy in BEIDEN Containern (app `…104639029998`, worker `…104639052590`) via `docker exec printenv` verifiziert = eu-prefixed; der Worker loggt beim Boot `LLM_MODEL=eu.anthropic.claude-sonnet-4-20250514-v1:0` + `Environment validated`, keine `invalid model identifier`-Fehler. Root-Cause (roher Env-Override des korrekten Code-Defaults aus `src/lib/llm.ts`) ist damit entfernt. Hinweis: eine separate Live-Invoke-Bestaetigung (echter V9/V10-Bedrock-Job) ist deploy-time nicht getriggert worden (Internal-Test-Stille); der naechste reale LLM-Job exerziert den Pfad. Founder hat die ENV am 2026-06-29 im Coolify-UI gesetzt.
- Severity: High
...
- Status: resolved
- Severity: Medium
- Resolution: 2026-06-17 (Code-side, RPT-485). Geschlossen durch SLC-V9.75-A Tier-Gating Foundation: `capture_session.tier` (free/blueprint/handbook, server-side erzwungen) + Matrix-Single-Source `fn_min_tier_for_job`/`fn_session_tier_allows` + Dispatch-Gates (Schicht 1) an allen gated Eintrittspunkten + Worker-Defense (Schicht 2) + Column-Level-Change-Guard + Anti-Forge-Insert-Guard. Ein blueprint/free-Mandant kann `email_bulk_*`/`sop_generation`/`handbook_snapshot_generation`/`dialogue_*`/`walkthrough_*`/`bridge_generation` weder per Menue noch per direktem RPC/Action/INSERT ausloesen (Bypass-Matrix GREEN, 53 Tests). Live-Apply Mig 121 erfolgt im /deploy V9.75.
...
- Next Action: **RESOLVED + DEPLOYED 2026-07-09 (REL-043, RPT-620, SOURCE_COMMIT `cb72df1`).** Fix: `src/lib/supabase/middleware.ts` redirectet strategaize_berater von `/dashboard` + `/capture` nach `/admin/mein-tag` (spiegelt employee/partner_admin-Muster; Matrix role-check.ts + defaultLandingForRole waren bereits die getestete SoT). Live-Smoke PASS: Berater-Login rendert Berater-Workspace „Mein Tag" statt Mandanten-Dashboard.
...
- Summary: Beim Rollen-Flow-E2E-Live-Durchlauf (2026-07-09, RPT-617) als `tenant_admin` fuehrte "Neue Erhebung starten" (`/capture/new`) zu einem Server-Fehler ("This page couldn't load", Next.js-Digest 1883366883). Serverseitig ein **ZodError** in `listTemplates()` (`src/lib/db/template-queries.ts:86-96`), das ALLE Zeilen der `template`-Tabelle gegen `TemplateRowSchema` parst (`TemplateBlockSchema`: `id` string, `title` als i18n-Record `z.record`, `questions[].id`). **Live-DB-Befund (2026-07-09): 2 von 22 template-Rows haben Legacy-Format** — `partner_diagnostic` (v1, 2026-05-16: `title` String statt Objekt, keine Block-/Fragen-IDs) und `exit-readiness-teaser-v1` (2026-05-29: `blocks[0].title` fehlt ganz, keine IDs). `listTemplates` wirft beim ERSTEN ungueltigen Row → die Seite crasht.
- Impact: **Global — betrifft JEDEN `tenant_admin`, nicht nur frische Mandanten** (der Picker listet templates tenant-unabhaengig). Der Direkt-Strang „Neue Erhebung" ist aktuell in Produktion komplett gebrochen. Der frische Test-GF war nur der erste Aufruf seit dem Seeding der 2 Legacy-Rows. Die 2 Uebeltaeter sind ein Diagnose-Seed (`093_v63_partner_diagnostic_seed`) + ein Teaser — gehoeren vermutlich gar nicht in den Erhebungs-Picker.
- Workaround: keiner (Seite laedt nicht).
- Next Action: **Code-Fix DONE 2026-07-09 (RPT-618)** — `listTemplates` (`src/lib/db/template-queries.ts`) macht jetzt per-Row `safeParse`: ungueltige Rows werden uebersprungen + via `captureWarning` (slug + zod-issues) geloggt, statt die ganze Seite zu werfen. Damit fallen die 2 Legacy-Rows automatisch aus dem Picker und die Loesung ist zukunftssicher gegen weiteren Legacy-Drift. 4 hermetische Vitest-Tests (`__tests__/template-queries-resilience.test.ts`), tsc 0 / eslint 0 / next build PASS / Full-Suite +4 pass, 0 Regression. **RESOLVED 2026-07-09 — DEPLOYED (REL-042, RPT-619, SOURCE_COMMIT `da5fb7c`).** Live-Smoke PASS: temp tenant_admin → `/capture/new` rendert Template-Picker statt Crash, die 2 Legacy-Rows korrekt absent. Folge-Produktfrage (Founder 2026-07-09 entschieden: „GF soll nichts von StB sehen, unterschiedliche Produkte") → **GELOEST + DEPLOYED (REL-043, RPT-620): `listTemplates` schliesst `stb_*`-Slugs aus dem generischen Picker aus; Live-Smoke `stbVisible=false`.** Optional-Rest: die 2 Legacy-Rows (partner_diagnostic/teaser) normalisieren/entfernen, falls nirgends sonst konsumiert (nicht dringend, safeParse deckt sie ab).
