...
- `template` (MIG-021, MIG-051): `blocks jsonb` (`blocks[].questions[]` mit `id`, `frage_id`, `owner_dependency:boolean` + 4 Flags — Schema `template-queries.ts:6-18`), `diagnosis_schema jsonb` (`blocks[].subtopics[].question_keys[]`, MIG-051 für Exit-Readiness geseedet).
